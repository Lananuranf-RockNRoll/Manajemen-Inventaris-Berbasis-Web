    ...
    <div class="card overflow-hidden">
      ...
      <template v-else>
        ...
        <div class="sm:hidden divide-y divide-zinc-800">
          <div v-for="trx in transactions" :key="trx.id" class="p-4 hover:bg-zinc-800/30 transition-colors">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 flex-1">
                ...
                <p class="text-sm font-bold text-zinc-100 mt-1 tabular-nums">{{ fmtUSD(trx.total_amount) }}</p>
              </div>
          ...
          <div v-if="transactions.length === 0" class="py-12 text-center text-zinc-500 text-sm">Tidak ada transaksi ditemukan.</div>
        </div>
        ...
        <div class="modal-box sm:max-w-2xl">
          ...
          <form @submit.prevent="handleCreate" class="space-y-4">
              ...
              <div class="space-y-2">
                <div v-for="(item, i) in createForm.items" :key="i" class="flex gap-2 items-center">
                  ...
                  <div class="relative shrink-0">
                    ...
                    <input v-model.number="item.unit_price" type="number" step="0.01" placeholder="0.00" required class="input-field w-24 sm:w-32 pl-5" />
                  </div>
                  <button type="button" @click="removeItem(i)" :disabled="createForm.items.length <= 1" class="text-red-400 hover:text-red-300 p-1 disabled:opacity-30 shrink-0">
                    <X class="w-3.5 h-3.5" />
                  </button>
                </div>
              </div>
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" :disabled="submitting" class="btn-primary">{{ submitting ? 'Memproses...' : 'Buat Order' }}</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div v-if="showDetailModal && detailTrx" class="modal-overlay" @click.self="showDetailModal = false">
        <div class="modal-box sm:max-w-2xl">
          ...
          <div class="grid grid-cols-2 gap-3 mb-5">
            <div><p class="text-xs text-zinc-500">Customer</p><p class="text-sm font-semibold text-zinc-200 mt-0.5">{{ detailTrx.customer?.name }}</p></div>
            <div><p class="text-xs text-zinc-500">Gudang</p><p class="text-sm font-semibold text-zinc-200 mt-0.5">{{ detailTrx.warehouse?.name }}</p></div>
            <div><p class="text-xs text-zinc-500">Tanggal Order</p><p class="text-sm text-zinc-300 mt-0.5">{{ detailTrx.order_date }}</p></div>
            <div><p class="text-xs text-zinc-500">Total</p><p class="text-sm font-bold text-indigo-400 mt-0.5 tabular-nums">{{ fmtUSD(detailTrx.total_amount) }}</p></div>
          </div>
              ...
              <table class="w-full">
                ...
                <tbody>
                  <tr v-for="item in detailTrx.items" :key="item.id" class="border-b border-zinc-700/50">
                    ...
                    <td class="td text-xs text-right text-zinc-400 tabular-nums whitespace-nowrap">{{ fmtUSD(item.unit_price) }}</td>
                    <td class="td text-xs text-right font-semibold text-zinc-200 tabular-nums whitespace-nowrap">{{ fmtUSD(item.subtotal) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
function nextStatuses(current: string): { value: string; label: string; class: string }[] {
  const map: Record<string, any[]> = {
    pending:    [{ value: 'processing', label: '→ Proses',   class: 'bg-blue-950 text-blue-300 border-blue-800 hover:bg-blue-900' }, { value: 'canceled', label: '✕ Batalkan', class: 'bg-red-950 text-red-300 border-red-800 hover:bg-red-900' }],
    processing: [{ value: 'shipped',    label: '→ Kirim',    class: 'bg-indigo-950 text-indigo-300 border-indigo-800 hover:bg-indigo-900' }, { value: 'canceled', label: '✕ Batalkan', class: 'bg-red-950 text-red-300 border-red-800 hover:bg-red-900' }],
    shipped:    [{ value: 'delivered',  label: '✓ Selesai',  class: 'bg-emerald-950 text-emerald-300 border-emerald-800 hover:bg-emerald-900' }, { value: 'canceled', label: '✕ Batalkan', class: 'bg-red-950 text-red-300 border-red-800 hover:bg-red-900' }],
  }
  return map[current] ?? []
}
