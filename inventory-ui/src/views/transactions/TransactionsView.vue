<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      ...
      <!-- staff, manager, admin bisa buat order -->
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          <tr v-for="trx in transactions" :key="trx.id"
            ...
            <td class="td text-center">
              <div class="flex items-center justify-center gap-2">
                ...
                <!-- Hanya manager & admin yang bisa update status -->
                <button
      ...
      <div v-else-if="auth.isStaff && !auth.isManager" class="px-4 py-2 bg-zinc-800/30 border-t border-zinc-800 text-xs text-zinc-500">
        ℹ️ Sebagai <strong>staff</strong>, Anda dapat membuat order baru. Update status dilakukan oleh manager.
      </div>
    </div>
          ...
          <div>
            ...
            <div class="space-y-2">
              <div v-for="(item, i) in createForm.items" :key="i" class="flex gap-2 items-center">
                ...
                <input v-model="item.unit_price" type="number" step="0.01" min="0" placeholder="Harga" required class="input-field w-32" />
      ...
      <div class="modal-box max-w-2xl">
        ...
        <div class="grid grid-cols-2 gap-4 mb-5">
          <div>
            ...
            <p class="text-sm font-semibold text-zinc-200">{{ detailTrx.customer?.name }}</p>
          </div>
          <div>
            <p class="text-xs text-zinc-500">Gudang</p>
            <p class="text-sm font-semibold text-zinc-200">{{ detailTrx.warehouse?.name }}</p>
          </div>
          <div>
            <p class="text-xs text-zinc-500">Tanggal Order</p>
            <p class="text-sm text-zinc-300">{{ detailTrx.order_date }}</p>
          </div>
          <div>
            <p class="text-xs text-zinc-500">Total</p>
            <p class="text-sm font-bold text-indigo-400">Rp {{ formatPrice(detailTrx.total_amount) }}</p>
          </div>
        </div>
...
import { Plus, Eye, RefreshCw, X } from 'lucide-vue-next'
...
async function handleCreate() {
  ...
  try {
    ...
    formError.value = errors ? Object.values(errors).flat().join(', ') : e.response?.data?.message ?? 'Gagal membuat order'
  } finally {
